...
def train(epoch: int, dataset: Dataset):
  model.train()
  out: Tensor = model(dataset.samples)
  loss_components = loss_fn(out, dataset.latents)
...
def test():
  ...
  samples: IntTensor = get_resized_samples(test_samples_dir, processed_test_data_dir, get_sample_filenames)
  samples: FloatTensor = samples.to(training_dtype)
  samples = samples-int8_half_range
  samples = samples/int8_half_range

  true_latents: FloatTensor = get_latents(test_latents_dir, processed_test_data_dir, get_latent_filenames)
  true_latents = true_latents.to(training_dtype)

  model.eval() # might be redundant due to inference mode but whatever

  predicted_latents: Tensor = model.forward(samples)
