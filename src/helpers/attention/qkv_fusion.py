...
def fuse_qkv(attn: Attention, fuse_scale_factor=False) -> None:
    ...
    qkv.weight.data.copy_(torch.cat([
        (attn.to_q.weight.data * attn.scale) if fuse_scale_factor else attn.to_q.weight.data,
        attn.to_k.weight.data,
        attn.to_v.weight.data
    ]))
    if has_bias:
        qkv.bias.data.copy_(torch.cat([
            (attn.to_q.bias.data * attn.scale) if fuse_scale_factor else attn.to_q.weight.data,
            attn.to_k.bias.data,
            attn.to_v.bias.data,
        ]))
